...
def softmax(matin):
    #see = np.amax(matin, axis=1)
    #see2 = (matin.T-np.amax(matin, axis=1)).T
    #a = np.exp(matin-np.amax(matin, axis=1))
    #b = np.sum(np.exp(matin-np.amax(matin, axis=1)),axis=1)
    #c = a/b
    #return np.exp((matin.T-np.amax(matin, axis=1)).T)/np.sum((matin.T-np.amax(matin, axis=1)).T)

#def softmaxorig(matin):
    mat = np.zeros(matin.shape)
    rows, cols = mat.shape
    #print(mat.shape)
    #print("before e:",matin)
    for r in range(rows):
        for c in range(cols):
            mat[r][c] = np.exp(matin[r][c]-np.ndarray.max(matin[r]))
    #print("e:",mat)
    for r in range(rows):
        softmaxsum = 0
        for cc in mat[r]:
